/**
 * Wykonuje inicjalizację strony po załadowaniu DOM.
 */
document.addEventListener('DOMContentLoaded', () => {
  /**
   * Pole tekstowe do wprowadzania kodu HTML.
   * @type {HTMLElement}
   */
  const htmlCode = document.getElementById('html-code');

  /**
   * Pole tekstowe do wprowadzania kodu CSS.
   * @type {HTMLElement}
   */
  const cssCode = document.getElementById('css-code');

  /**
   * Element ramki podglądu.
   * @type {HTMLIFrameElement}
   */
  const outputFrame = document.getElementById('output');

  /**
   * Element informacji zwrotnej.
   * @type {HTMLElement}
   */
  ...
  /**
   * Element ramki iframe dla podglądu.
   * @type {HTMLIFrameElement}
   */
  const iframe = document.getElementById('output');

  /**
   * Przycisk otwierający poradnik.
   * @type {HTMLElement}
   */
  const poradnik = document.getElementById('Poradnik');

  /**
   * Element odtwarzający dźwięk przy kliknięciu.
   * @type {HTMLAudioElement}
   */
  const buttonSound = document.getElementById('button-sound');

  /**
   * Obsługuje kliknięcie przycisku "Poradnik".
   */
  poradnik.addEventListener('click',openSite);

  /**
   * Określa, czy podgląd jest w trybie pełnoekranowym.
   * @type {boolean}
   */
  let isFullScreen = false;
  /**
   * Rejestruje kliknięcie przycisku "Przejdź do kolejnego poziomu".
   */
  nextLevelButton.addEventListener('click', goToNextLevel);

  /**
   * Rejestruje kliknięcie przycisku resetu gry.
   */
  reset.addEventListener('click', resetGame);

   /**
   * Rejestruje kliknięcie przycisku pełnego ekranu.
   */
...
/**
   * Funkcja efektu przy pojawieniu się przycisku
   * 
   */
...
function updatePreview() {
   ...
   * 
   *//**
 * Czyści kod, usuwając zbędne spacje i konwertując go na małe litery.
 * @param {string} code - Kod do oczyszczenia.
 * @returns {string} - Oczyszczony kod.
 */
...
/**
* Zapisuje aktualne dane (HTML, CSS, poziom gry) do localStorage.
*/
...
/**
* Ładuje dane (HTML, CSS) z localStorage i ustawia je w edytorach.
*/
...
/**
* Ustawia komunikat o sukcesie w sekcji informacji zwrotnej.
...
/**
* Ustawia komunikat o błędzie w sekcji informacji zwrotnej.
* @param {string} message - Wiadomość do wyświetlenia.
*/
...
/**
* Resetuje grę, usuwa dane z localStorage i wywołuje animację okna potwierdzenia.
*/
function resetGame() {
...
/**
* Przełącza tryb pełnoekranowego podglądu sekcji podglądu.
*/
...
/**
 * Obsługuje kliknięcie przycisku pobierania zawartości HTML i CSS.
 */
document.getElementById('downloadBtn').addEventListener('click', function() {
  /**
   * Pobiera zawartość edytorów HTML i CSS.
   * @type {string}
   */
  const htmlContent = htmlEditor.getValue();

  /**
   * Pobiera zawartość edytorów CSS.
   * @type {string}
   */
  const cssContent = cssEditor.getValue();
  
  /**
   * Tworzy plik i rozpoczyna jego pobieranie.
...
/**
 * Sprawdza poprawność poziomu 1 (nagłówek H1).
 */
...
/**
 * Sprawdza poprawność poziomu 2 (wyśrodkowanie tekstu w CSS).
 */
...
/**
 * Sprawdza poprawność poziomu 3 (kolor tła w CSS).
 */
function checkLevel3() {
  ...
  /**
   * Wzorzec do sprawdzenia właściwości background-color w sekcji body.
   * @type {RegExp}
   */
  const bodyBackgroundColorTest = /body\s*\{[^}]*background-color\s*:\s*([^;]+)\s*;[^}]*\}/;

  /**
   * Wynik dopasowania koloru w CSS.
   * @type {RegExpMatchArray | null}
   */
  const match = css.match(bodyBackgroundColorTest);

  /**
   * Sprawdza, czy dany kolor jest w poprawnym formacie (HEX lub RGB).
   * @param {string} color - Kolor do sprawdzenia.
   * @returns {boolean} - Czy kolor jest poprawny.
   */
  ...
  /**
   * Sprawdza, czy kolor nie jest czarny (zabroniony).
   * @param {string} color - Kolor do sprawdzenia.
   * @returns {boolean} - Czy kolor nie jest czarny.
   */
...
/**
 * Sprawdza poprawność poziomu 4 (dodanie obrazka).
 */
function checkLevel4() {
  ...
  /**
   * Wzorzec dla znacznika obrazu <img>.
   * @type {RegExp}
   */
...
/**
 * Sprawdza poprawność poziomu 5 (lista uporządkowana i styl CSS).
 */
function checkLevel5() {
  ...
  /**
   * Wzorzec dla listy uporządkowanej z trzema elementami.
   * @type {RegExp}
   */
  const olPattern = /<ol>\s*<li>.*<\/li>\s*<li>.*<\/li>\s*<li>.*<\/li>\s*<\/ol>/s;

  /**
   * Czy lista spełnia wymagania.
   * @type {boolean}
   */
  const olValid = olPattern.test(html);

  /**
   * Czy CSS zawiera styl numeracji "upper-roman".
   * @type {boolean}
   */
  const cssValid = /list-style-type:\s*upper-roman\s*;/.test(css);

  /**
   * Sprawdza, czy wszystkie bloki CSS mają odpowiednio zamknięte klamry.
   * @param {string} cssCode - Kod CSS do sprawdzenia.
   * @returns {boolean} - Czy struktura CSS jest poprawna.
   */
  ...
  /**
 * Sprawdza poprawność poziomu 6 (lista nieuporządkowana z czterema elementami <li> i stylem 'circle').
 */
function checkLevel6() {
  ...
  /**
   * Wzorzec dla nieuporządkowanej listy z czterema elementami <li>.
   * @type {RegExp}
   */
  const ulPattern = /<ul>\s*<li>.*<\/li>\s*<li>.*<\/li>\s*<li>.*<\/li>\s*<li>.*<\/li>\s*<\/ul>/s;
  ...
  /**
   * Czy CSS zawiera styl 'circle' dla listy.
   * @type {boolean}
   */
  const cssValid = /list-style-type:\s*circle\s*;/.test(css);

  /**
   * Sprawdza, czy wszystkie sekcje CSS mają odpowiednio zamknięte klamry.
   * @param {string} cssCode - Kod CSS do sprawdzenia.
   * @returns {boolean} - Czy struktura CSS jest poprawna.
   */
  function validateCSSBlocks(cssCode) {
...
/**
 * Sprawdza poprawność poziomu 7 (dodanie poprawnego linku HTML).
 */
function checkLevel7() {
  ...
  /**
   * Wzorzec dla poprawnego linku <a>.
   * @type {RegExp}
   */
...
/**
 * Sprawdza poprawność poziomu 8 (stworzenie struktury nagłówka z sekcjami HTML).
 */
function checkLevel8() {
  ...
  /**
   * Wzorce dla wymaganych elementów HTML (header, nav, ul, li, a, h1, p).
   * @type {RegExp}
   */
  ...
  /**
   * Sprawdza, czy wszystkie wymagane elementy HTML istnieją w kodzie.
   * @type {boolean}
   */
...
/**
 * Sprawdza poprawność poziomu 9 (co najmniej jedna sekcja HTML).
 */
function checkLevel9() {
  ...
  /**
   * Wzorzec dla sekcji HTML.
   * @type {RegExp}
   */
  const sectionTest = /<section[^>]*>[\s\S]*<\/section>/g;

  /**
   * Dopasowane sekcje HTML.
   * @type {Array<string> | null}
   */
  const sections = currentHTML.match(sectionTest);

  /**
   * Czy kod zawiera co najmniej jedną sekcję HTML.
   * @type {boolean}
   */
...
/**
 * Sprawdza poprawność poziomu 10 (wszystkie sekcje wypełnione).
 */
function checkLevel10() {
  /**
   * Parsuje kod HTML do obiektu DOM.
   * @type {Document}
   */
  const currentHTML = new DOMParser().parseFromString(htmlEditor.getValue(), 'text/html');

  /**
   * Lista wszystkich sekcji w dokumencie.
...
/**
 * Sprawdza poprawność poziomu 11 (stopka HTML).
 */
function checkLevel11() {
  ...
  /**
   * Czy kod zawiera poprawną strukturę stopki HTML.
   * @type {boolean}
   */
...
/**
 * Sprawdza poprawność poziomu 12 (kolor tła w CSS).
 */
function checkLevel12() {
  ...
  /**
   * Wzorzec dla background-color w sekcji body.
   * @type {RegExp}
   */
  const bodyBackgroundColorTest = /body\s*\{[^}]*background-color\s*:\s*([^;]+)\s*;[^}]*\}/;

  /**
   * Dopasowanie koloru w CSS.
   * @type {RegExpMatchArray | null}
   */
  const match = css.match(bodyBackgroundColorTest);
...
/**
 * Sprawdza poprawność poziomu 13 (wszystkie wymagane właściwości w sekcji body CSS).
 */
function checkLevel13() {
  ...
  /**
   * Wzorzec dla sekcji body z wymaganymi właściwościami.
   * @type {RegExp}
   */
  const bodyTest = /body\s*\{\s*(?=.*font-family:\s*[^;]+;)(?=.*margin:\s*[^;]+;)(?=.*padding:\s*[^;]+;)(?=.*background-color:\s*[^;]+;)[^}]*\}/;

  /**
   * Sprawdza, czy wartość font-family jest poprawna.
   * @param {string} value - Wartość font-family.
   * @returns {boolean} - Czy font-family jest poprawny.
   */
  ...
  /**
   * Sprawdza, czy wartości margin/padding są poprawne.
   * @param {string} value - Wartość margin lub padding.
   * @returns {boolean} - Czy wartość jest poprawna.
   */
  ...
  /**
   * Sprawdza, czy właściwości sekcji body są poprawne.
   * @param {string} css - Kod CSS sekcji body.
   * @returns {boolean} - Czy wszystkie właściwości są poprawne.
   */
  function isValidBodyProperties(css) {
      ...
      return (
          ...
          backgroundColorMatch && isValidColor(backgroundColorMatch[1].trim())
      );
  }

  /**
   * Czy kod CSS sekcji body zawiera wszystkie poprawne właściwości.
   * @type {boolean}
   */
...
/**
 * Sprawdza poprawność poziomu 14 (CSS dla header, section i footer).
 */
function checkLevel14() {
  ...
  /**
   * Waliduje poprawność nawiasów klamrowych w kodzie CSS.
   * @param {string} cssCode - Kod CSS do sprawdzenia.
   * @returns {boolean} - Czy wszystkie nawiasy klamrowe są poprawnie zamknięte.
   */
  function validateCSSBlocks(cssCode) {
    const openBraces = (cssCode.match(/\{/g) || []).length;
    const closeBraces = (cssCode.match(/\}/g) || []).length;
  ...
  // Pozostałe walidacje w checkLevel14 zostały odpowiednio opisane w formie funkcji.
}

/**
 * Sprawdza poprawność poziomu 15 (sekcja z tabelą 3x3 w HTML).
 */
function checkLevel15() {
  ...
  /**
   * Wzorzec dla sekcji HTML z tabelą 3x3.
   * @type {RegExp}
   */
  const tableRegex = /<section[^>]*>[\s\S]*<table[^>]*>[\s\S]*<tr>[\s\S]*<th>[\s\S]*<\/th>[\s\S]*<\/tr>[\s\S]*<tr>[\s\S]*<td>[\s\S]*<\/td>[\s\S]*<\/tr>[\s\S]*<\/table>[\s\S]*<\/section>/g;
...
/**
 * Sprawdza poprawność poziomu 16 (style nawigacji w CSS).
 */
function checkLevel16() {
  ...
  /**
   * Wzorce dla testów CSS nawigacji.
   * @type {RegExp}
   */
...
/**
 * Sprawdza poprawność poziomu 17 (style tabel w CSS).
 */
function checkLevel17() {
  ...
  /**
   * Wzorce dla tabel i nagłówków w CSS.
   * @type {RegExp}
   */
...
/**
 * Sprawdza poprawność poziomu 18 (formularz HTML z wymaganymi polami).
 */
...
/**
 * Sprawdza poprawność poziomu 19 (style formularza w CSS).
 */
function checkLevel19() {
  ...
  /**
   * Wzorce dla stylów formularza, inputów i przycisków.
   * @type {RegExp}
   */
...
/**
 * Wyświetla komunikat końcowy po ukończeniu wszystkich poziomów.
 */
function checkLevel20() {
  /**
   * Komunikat końcowy dla użytkownika.
   * @type {string}
   */
  ...
  /**
   * Wyświetla przycisk pobierania kodu HTML i CSS.
   */
  downloadBtn.style.display = "block";

  /**
   * Ukrywa przycisk przejścia do kolejnego poziomu.
   */
  nextLevelButton.style.display = "none";
}

  /**
 * Funkcja zwiększająca poziom gry po naciśnięciu przycisku
 */
function goToNextLevel() {
    ...
    if (currentLevel === 2) {
      ...
      setInstructions(currentLevel);
      htmlEditor.setValue("<header> </header>"); // Ustawienie zawartości HTML
...
/**
 * Funkcja sprawdzająca zmiany w edytorze HTML na poszczególnych poziomach
 */
  ...
  /**
 * Funkcja sprawdzająca zmiany w edytorze CSS na poszczególnych poziomach
 */
  cssEditor.on('change', () => {
    updatePreview();
    if (currentLevel === 2) {
  ...
  /**
 * Funkcja otwiera stronę z instrukcjami dla bieżącego poziomu.
 */
  function openSite(){
    if (currentLevel === 1) {
 ...
 /**
  * Funkcja wyświetlająca instrukcje dla poszczególnych poziomów
 */
  setInstructions(currentLevel);
...
/**
 * Aktualizuj podgląd przy każdej zmianie w kodzie HTML
 */
  htmlCode.addEventListener('input', updatePreview);
  /**
 * Aktualizuj podgląd przy każdej zmianie w kodzie CSS
 */
  cssCode.addEventListener('input', updatePreview);
  /**
 * Wywołanie funkcji aktualizującej iframe przy każdej zmianie w kodzie
 */
  updatePreview();
